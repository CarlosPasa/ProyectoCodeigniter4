...
function vDateTimePicker2($id, $campoValor, $idDiv){
	echo '<div class="input-group date" id="' . $idDiv .'" data-target-input="nearest">';
	echo '	<input type="text" id="'. $id .'" name="' . $id .'" class="form-control datetimepicker-input"';
	echo ' 		value="'. $campoValor .'" data-target="#' . $idDiv .'">';
	echo '	<div class="input-group-append" data-target="#' . $idDiv .'" data-toggle="datetimepicker">
...
class modelFieldAction{
	...
	public static function btnEdit($url, $columnNameID){
		...
		return new modelFieldAction(array("url"=>$url."/_id_", "columnNameID"=>$columnNameID, "class"=>"btn btn-sm btn-primary", "icono"=>"fa fa-edit","tooltip"=>"Editar"));

	}

	public static function btnDelete($url, $columnNameID){

		return new modelFieldAction(array("url"=>$url."/_id_", "columnNameID"=>$columnNameID, "class"=>"btn btn-sm btn-danger", "icono"=>"fa fa-trash","tooltip"=>"Eliminar","onClick"=>"ShowDialogQuestion('PREGUNTA','¿Desea eliminar el registro seleccionado?','_url_')"));

	}
	// Creado el 08/03/2023
	public static function btnEditJS($js, $columnNameID){
		return new modelFieldAction(array("class"=>"btn btn-sm btn-info","icono"=>"fa fa-edit","tooltip"=>"Modificar","columnNameID"=>$columnNameID,"onClick"=>$js."('_id_')"));
	}
...
if(!function_exists('vCheckBoxSwitch')){

	function vCheckBoxSwitch($name, $textOn=null, $textOff=null, $colorOn=null, $colorOff=null, $checked=0){
		
		echo "<input type=\"checkbox\" id=\"". $name . "\" name=\"" . $name . "\" ";

		echo "data-bootstrap-switch data-off-color='danger' data-on-color='success'";
		
		if ($textOn !== null || $textOn !== "") echo " data-on-text='".$textOn."'";

		if ($textOff !== null || $textOff !== "") echo " data-off-text='".$textOff."'";

		if ($colorOn !== null || $colorOn !== ""){
			echo " data-on-color='".$colorOn."'";
		}else{
			echo " data-on-color='success'";
		}

		if ($colorOff !== null || $colorOff !== ""){
			echo " data-off-color='".$colorOff."'";
		}else{
			echo " data-off-color='danger'";
		}
...
if(!function_exists('vFotoThumbnail')){
	...
	function vFotoThumbnail($divName, $divTittle, $width, $height, $defaultImagePath, $showButtons = true, $checkBoxOmitir = false, $imageStyle = null){
		...
		if ($showButtons == true){
			...
			echo "<button type=\"button\" class=\"btn btn-danger pull-right\" onclick=\"" . $onRemove . "\"><i class=\"fa fa-trash\" aria-hidden=\"true\"></i></button>\n";	
